...
**Testable logic → plan it test-first (TDD).** When a phase delivers logic with a clear input→output contract — Bun services, Drizzle repositories, zod schemas, token/crypto/format utilities, pure store transitions — fold testing into that phase per the `tdd` skill (`.claude/skills/tdd/SKILL.md`), don't bolt it on. Concretely:
