> Source: https://github.com/blackboardsh/electrobun/blob/main/docs/src/content/docs/electrobun/apis/application-icons.mdx · fetched 2026-05-24
...
You can specify a custom path for the `icon.iconset` folder in your [electrobun.config](/electrobun/apis/cli/build-configuration) file.
...
> **Tip:** **Important:** Building `.icon` files requires the full Xcode installation because Electrobun uses `actool`. Command Line Tools alone are not enough. If you do not have Xcode installed, use an `icon.iconset` folder instead.

### Windows

Set the `build.win.icon` option in your [electrobun.config](/electrobun/apis/cli/build-configuration) file to a path to an `.ico` or `.png` file. If you provide a PNG, it will be automatically converted to ICO format during the build.
...
Set the `build.linux.icon` option in your [electrobun.config](/electrobun/apis/cli/build-configuration) file to a path to a `.png` file. The icon should be at least 256x256 pixels.
...
> **Tip:** **Tip:** You can reuse PNGs from your macOS `icon.iconset` folder for Windows and Linux builds, so you don't need to maintain separate icon files per platform.
