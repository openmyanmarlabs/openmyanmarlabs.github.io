> Source: https://github.com/blackboardsh/electrobun/blob/main/docs/src/content/docs/electrobun/apis/events.mdx · fetched 2026-05-24
...
Fired on macOS when the application is opened via a custom URL scheme or an associated file. File opens arrive as `file://` URLs through this same event.
...
**Setup:** To register deep links or file associations for your app, add `urlSchemes` and/or `fileAssociations` to your `electrobun.config.ts`. See the [Build Configuration](/electrobun/apis/cli/build-configuration) docs for details.
...
> **Tip:** **Linux note:** On Linux, system-initiated quit paths (Ctrl+C, window manager close, taskbar quit) do not currently fire `before-quit`. Programmatic quit via `Utils.quit()` and `process.exit()` works correctly on all platforms.
...
> **Tip:** **Recommendation:** Use Electrobun's `before-quit` event for all shutdown cleanup. It fires for every quit path, supports async operations, and can cancel the quit. The native `process.on("exit")` can be used as a last-resort sync hook, but `process.on("beforeExit")` will not fire in Electrobun apps.
